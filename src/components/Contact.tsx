...
const contactInfo = [
  ...
  { icon: MapPin, label: "Location", value: "Ho Chi Minh City, Vietnam", href: undefined },
  { icon: Phone, label: "Phone", value: "039 393 0709", href: "tel:+84393930709" },
];
...
export default function Contact() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const subject = encodeURIComponent(`Liên hệ từ ${form.name || "website"}`);
        ...
        <div className="grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          >
            {contactInfo.map(({ icon: Icon, label, value, href }) => {
              ...
              return href ? (
                <a key={label} href={href} className="block hover:opacity-80 transition-opacity">
                  {content}
                </a>
              ) : (
